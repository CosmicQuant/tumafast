            ...
            <div className="max-w-7xl mx-auto px-4 relative z-10 w-full overflow-visible">
                ...
                <div className="flex flex-col gap-12 w-full">
                    {carriers.map((carrier, idx) => {
                        ...
                        return (
                            <div key={carrier.title} className="relative w-full group scroll-reveal anim-up" style={{ animationDelay: \`\${idx * 0.2}s\` }}>
                                ...
                                */}
                                ...
                                <div 
                                    ...
                                    style={{
                                        left: 0,
                                        animation: \`vehicle-drive \${carrier.animationDuration} linear infinite \${carrier.animationDelay}\`
                                    }}
                                >
                                    <div className={\`w-10 h-10 rounded-full bg-gradient-to-br \${carrier.gradient} flex items-center justify-center shadow-[0_0_20px_rgba(0,0,0,0.5)]\`}>
                                        <Icon className="w-5 h-5 text-white" />
                                    </div>
                                    {/* Trail effect */}
                                    <div className={\`h-[2px] w-24 bg-gradient-to-r \${carrier.gradient} scale-opacity opacity-50 blur-sm -ml-2 rounded-full\`} style={{ right: '100%', position: 'absolute' }} />
                                </div>

                                {/* Content sitting OVER the track */}
                                <div className={\`relative z-10 bg-slate-900/60 backdrop-blur-xl rounded-[2.5rem] p-8 md:p-10 border border-white/10 overflow-hidden transition-all duration-500 hover:border-white/20 hover:bg-slate-900/80 group-hover:shadow-[0_0_40px_rgba(0,0,0,0.3)]\`}>
                                    
                                    {/* Glowing aura inside */}
                                    <div className={\`absolute -top-32 -right-32 w-64 h-64 bg-gradient-to-br \${carrier.gradient} rounded-full blur-[100px] opacity-10 group-hover:opacity-30 transition-opacity duration-700\`} />

                                    <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start lg:items-center">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-4 mb-4">
                                                <div className={\`w-16 h-16 rounded-2xl bg-white/[0.03] border \${carrier.accentBorder} flex items-center justify-center \${carrier.accentText} group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500\`}>
                                                    <Icon className="w-8 h-8" />
                                                </div>
                                                <div>
                                                    <div className={\`text-xs font-bold uppercase tracking-widest \${carrier.accentText} mb-1 opacity-80\`}>
                                                        {carrier.subtitle}
                                                    </div>
                                        ...
                                        <div className="flex-shrink-0 w-full lg:w-auto flex flex-col sm:flex-row lg:flex-col gap-6">
                                            <div className="space-y-3">
                                                {carrier.perks.map((perk) => (
                                                    <div key={perk} className="flex items-center gap-3">
                                                        <div className={\`w-6 h-6 rounded-full \${carrier.accentBg} flex items-center justify-center flex-shrink-0\`}>
                                                            <div className={\`w-2 h-2 rounded-full bg-gradient-to-br \${carrier.gradient}\`} />
                                                        </div>
                                            ...
                                            <button className={\`inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-white/5 border border-white/10 text-white font-semibold hover:bg-white/10 hover:border-white/20 transition-all duration-300 group/btn\`}>
                                                {carrier.cta}
                                                <ArrowRight className={\`w-4 h-4 \${carrier.accentText} md:group-hover/btn:translate-x-1 transition-transform\`} />
                                            </button>
                                        </div>
                                    </div>
                                </div>
